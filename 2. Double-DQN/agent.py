...
class Agent:
    ...
    def update(self):
        ...
        """
        Compared to DQN, only this part needs to be modified for Double-DQN
        """
        max_q_net_action = self.q_net(next_states).max(1).indices.unsqueeze(1)
        with torch.no_grad():
            max_target_q_net = self.target_q_net(next_states).gather(1, max_q_net_action)
